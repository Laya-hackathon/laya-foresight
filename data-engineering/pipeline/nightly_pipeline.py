...
if not DATABASE_URL:
    log.error("DATABASE_URL not found. Create a .env file in laya-data/")
    sys.exit(1)
...
# ─────────────────────────────────────────────────────────────────────────────
def get_active_claims(run_date, conn):
    """
...
# ─────────────────────────────────────────────────────────────────────────────
def get_existing_snapshots(run_date, conn):
    """
...
# ─────────────────────────────────────────────────────────────────────────────
def load_recent_events(claim_ids, run_date, conn):
    """
...
# ─────────────────────────────────────────────────────────────────────────────
def build_user_history(active_claims, run_date, conn):
    """
    ...
    all_calls = pd.read_sql(text(f"""
        ...
        JOIN claims c ON sc.claim_id = c.claim_id
        WHERE c.user_id IN ({ids_str})
    """), conn)
...
# ─────────────────────────────────────────────────────────────────────────────
def write_snapshots(snapshots, conn):
    """
    Inserts new snapshot rows into feature_snapshots.
...
# MAIN PIPELINE
# ─────────────────────────────────────────────────────────────────────────────
def run_pipeline(run_date=None):
    """
    Main pipeline entry point.
    Call with a specific run_date for backfilling historical dates.
    Leave as None to run for today.
    """
    if run_date is None:
    ...
    with engine.connect() as conn:
        ...
        active_claims = get_active_claims(run_date, conn)
        ...
        existing = get_existing_snapshots(run_date, conn)
        ...
        logs_df, status_df, calls_df = load_recent_events(claim_ids, run_date, conn)
        ...
        user_history = build_user_history(claims_to_process, run_date, conn)
    ...
    with engine.begin() as write_conn:
        written = write_snapshots(snapshots, write_conn)
    log.info(f"  Written to feature_snapshots: {written:,} rows")
...
if __name__ == "__main__":
    ...
    if args.backfill:
        ...
        for i in range(args.backfill, 0, -1):
            ...
            run_pipeline(run_date=backfill_date)
        ...
        run_pipeline(run_date=specific_date)
